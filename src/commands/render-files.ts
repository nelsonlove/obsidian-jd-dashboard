...
export async function renderFiles(
	...
	if (!taskModel) {
		new Notice("Render Files: no model configured. Set one under Per-task models in plugin settings.");
		return;
	}
	const apiKey = getApiKey(app, taskModel.provider);
	if (!apiKey) {
		new Notice(`Render Files: no API key set for ${taskModel.provider}.`);
		return;
	}

	const fsPath = resolveFilesystemPath(settings, file, id);
	if (!fsPath) {
		new Notice(`Render Files: could not resolve filesystem path for ${id}.`);
		return;
	}

	let entries: ListedEntry[];
	try {
		entries = listDirectory(fsPath);
	} catch (e) {
	...
	const prompt = buildPrompt(settings, fsPath, entries);
	...
	const wrapped = wrapAutoGenerated(body, fsPath, entries.length, taskModel);
	const original = await app.vault.read(file);
	const updated = setSection(original, SECTION_HEADING, wrapped);

	if (updated === original) {
		// Re-running the command IS a survey event — bump even on no-op.
		await markSurveyed(app, file);
		new Notice("Render Files: no body changes — bumped `surveyed`.");
		return;
	}
	await app.vault.modify(file, updated);
	await markSurveyed(app, file);
	new Notice("Render Files: rendered.");
}
...
function resolveFilesystemPath(settings: JDSettings, file: TFile, _id: string): string | null {
	const home = process.env.HOME ?? "";
	let root = settings.jdRoot.replace("~", home);
	while (root.length > 1 && root.endsWith(sep)) root = root.slice(0, -1);
	if (!root.startsWith(sep)) return null;
	...
	if (!vaultRelDir) return null;

	const segments = vaultRelDir.split("/");
	if (segments.some((s) => s === ".." || s === "")) return null;
	if (vaultRelDir.startsWith("/")) return null;
...
function listDirectory(absPath: string): ListedEntry[] {
	let root = absPath;
	while (root.length > 1 && root.endsWith(sep)) root = root.slice(0, -1);
	...
	for (const name of names) {
		...
		if (lst.isSymbolicLink()) {
			...
			try { target = readlinkSync(full); } catch { target = undefined; }
			out.push({ name, kind: "symlink", target });
		} else if (lst.isDirectory()) {
			out.push({ name, kind: "dir" });
		} else {
			out.push({ name, kind: "file", size: lst.size });
		}
	}
...
function formatListing(entries: ListedEntry[]): string {
	...
	for (const e of entries) {
		if (e.kind === "symlink") {
			rows.push(`${e.name}    → ${e.target ?? "?"} (symlink)`);
		} else if (e.kind === "dir") {
			rows.push(`${e.name}/    (dir)`);
		} else {
			rows.push(`${e.name}    ${formatSize(e.size ?? 0)}`);
		}
	}
	return rows.join("\n");
}
