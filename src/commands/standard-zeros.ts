 ...
 * category folder (basename starts with `XX `).
 */
...
export async function standardZerosCommand(app: App, file: TFile): Promise<void> {
	...
	const { created, skipped } = await createStandardZeros(app, folder as TFolder, prefix, now);
	new Notice(`Created ${created}, skipped ${skipped} existing`);
}
